...
urlpatterns = [
    ...
    url(r'^dashboard/(\d+)$', login_required(views.DoctorDashboard.as_view()), name='dashboard'),
    url(r'^dashboard/(\d+)/update$', login_required(views.DoctorDashboardUpdate.as_view()), name='update_dashboard'),
    ...
    url(r'^dashboard/calculate_wait_time/(\d+)$', views.CalculateWaitTime.as_view(), name='calculate_wait'),
]
